...
/// Find a cliente by the cliente's authentication information (email + password)
/// Return an Unauthorized error if it doesn't match
pub fn find_by_auth(
    pool: &PoolType,
    cliente_email: &str,
    cliente_password: &str,
) -> Result<ClienteResponse, ApiError> {
    use crate::schema::clientes::dsl::{email, password, clientes};

    let mut conn = pool.get()?;
    let cliente = clientes
        .filter(email.eq(cliente_email.to_string()))
        .filter(password.eq(cliente_password.to_string()))
        .first::<Cliente>(&mut conn)
...
impl From<NewCliente> for Cliente {
    fn from(cliente: NewCliente) -> Self {
        Cliente {
            ...
            email: cliente.email,
            password: hash(&cliente.password),
...
pub mod tests {
    ...
    pub fn create_cliente() -> Result<ClienteResponse, ApiError> {
        ...
        let new_cliente = NewCliente {
            ...
            nome_social: Some("aii bofe".to_string()),
            cpf: Some("12345678901".to_string()),
            rg: Some("123456789".to_string()),
            passaporte: Some("123456789".to_string()),
    ...
    fn it_updates_a_cliente() {
        ...
        let update_cliente = UpdateCliente {
            ...
            updated_by: cliente.id.to_string(),
        };
